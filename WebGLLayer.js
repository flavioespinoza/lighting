...
function WebGLLayer(map) {
   ...
   * @private {number}
   */
  this.resolutionScale_ = window.devicePixelRatio || 3;
...
WebGLLayer.prototype.changePointSize = function (currentZoom) {
  ...
  if (currentZoom > 18 || currentZoom < 12) {

  } else {
    if (currentZoom == 12) { pointSize = 2; }
    else if (currentZoom == 13) { pointSize = 2; }
    else if (currentZoom == 14) { pointSize = 3; }
    else if (currentZoom == 15) { pointSize = 4; }
    else if (currentZoom == 16) { pointSize = 5; }
...
WebGLLayer.prototype.update = function () {
  ...
  // gl.vertexAttribPointer(pointProgram.attributes.aPointSize, 4, gl.FLOAT, false, 12, 8);
  ...
  gl.aPointSize = gl.getAttribLocation(pointProgram.program, "aPointSize");
...
WebGLLayer.prototype.start = function () {
  ...
  var currentZoom = this.map_.zoom;
  var pointSize = Math.max(currentZoom - 30.0, 2.0);
